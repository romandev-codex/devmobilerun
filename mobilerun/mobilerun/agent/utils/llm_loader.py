...
# Profiles that are loaded when the config defines them, but never required:
# a config written before they existed must keep working.
_OPTIONAL_PROFILES: tuple[str, ...] = ("vision",)


def _get_optional_profiles(config: MobileConfig) -> List[str]:
    """Return optional profiles that are actually present in *config*."""
    return [name for name in _OPTIONAL_PROFILES if name in config.llm_profiles]
...
def load_agent_llms(
    ...
    **kwargs: Any,
    ...
    Returns:
        ...
        - vision is included when the config defines a "vision" profile
    ...
    profile_names = profile_names + _get_optional_profiles(config)
...
def merge_llms_with_config(
    ...
    **kwargs: Any,
    ...
    wanted = required + _get_optional_profiles(config)

    missing = [name for name in wanted if name not in llms]
    if not missing:
        return llms

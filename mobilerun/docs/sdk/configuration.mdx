...
FastAgentConfig(
    ...
    history_steps=15,                                          # Recent steps sent verbatim (0 = full history)
    summarize_history=True,                                    # Recap older steps instead of dropping them
...
**`see_screen`:** registered only when a `vision` LLM profile is configured. It takes a screenshot and asks that model a question about the screen, so a text-only agent can inspect images, icons, or unlabeled UI. Drop the `vision` profile, or add `see_screen` to `disabled_tools`, to turn it off.
...
agent = MobileAgent(
    ...
    llms={
        ...
        "vision": load_llm("GoogleGenAI", model="gemini-3.7-flash"),  # see_screen tool
    }
)
```
...
- `vision` - Backs the `see_screen` tool, which lets a text-only agent ask a
  vision model what is on the current screen. Must be a vision-capable model.
  Omit the key (and the `vision` config profile) to leave `see_screen`
  unregistered.
...
llm_profiles:
  ...
  vision:  # Backs the see_screen tool; must be a vision-capable model
    provider: GoogleGenAI
    model: gemini-3.7-flash
    temperature: 0.0
...
Set `MOBILERUN_OAUTH_MANUAL=true` to select manual authentication for Anthropic
and Gemini, or device-code authentication for OpenAI, instead of using a local
browser callback server. The accepted enabled values are `1`, `true`, and `yes`
(case-insensitive).

```bash
export MOBILERUN_OAUTH_MANUAL=true
```

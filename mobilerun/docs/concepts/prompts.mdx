...
Output format: optional `<thought>` or `<add_memory>` metadata may be included.
Use `<plan>` while work remains. Completed tasks use
`<request_accomplished success="true">`; blocked tasks use
...
When blocked:
<request_accomplished success="false">
Explain why the task cannot be completed.
</request_accomplished>

For final result tags, set the `success` attribute explicitly and quote its value.
"""
}
...
Optional `<thought>` or `<add_memory>` metadata may be included. Use `<plan>`
while work remains. Completed tasks use
`<request_accomplished success="true">`; blocked tasks use
`<request_accomplished success="false">`.
"""
}
...
Output: optional `<thought>` or `<add_memory>` metadata may be included. Use
`<plan>` while work remains. Completed tasks use
`<request_accomplished success="true">`; blocked tasks use
`<request_accomplished success="false">`.

...
**`see_screen`:** registered only when a `vision` LLM profile is configured. It takes a screenshot and asks that model to describe the screen, so a text-only agent can inspect images, icons, or unlabeled UI. Drop the `vision` profile, or add `see_screen` to `disabled_tools`, to turn it off.
